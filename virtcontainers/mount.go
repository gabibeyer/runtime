...
func bindMountContainerRootfs(ctx context.Context, sandboxID SandboxID, cID ContainerID, sharedDir, cRootFs string, readonly bool) error {
	span, _ := trace(ctx, "bindMountContainerRootfs")
	defer span.Finish()

	rootfsDest := filepath.Join(sharedDir, string(sandboxID), string(cID), rootfsDir)
...
func bindUnmountContainerRootfs(ctx context.Context, sharedDir string, sandboxID SandboxID, cID ContainerID) error {
